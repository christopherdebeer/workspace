...
export const RELIEF_MIN = 0.06;
/** Rolling carve cost: [tiles carved, total ms, tiles that needed relief]. */
export const carveCost = { tiles: 0, ms: 0, relieved: 0 };
const clamp = (v: number, a: number, b: number): number => Math.max(a, Math.min(b, v));
export const BANK_K = 0.6;         // a fill bank falls this much per metre out from the crest
export const CUTF_K = 0.62;        // a cut face rises this much per metre out — the carve's own slope
export const CUT_REACH_M = 8;      // past this an unmet cut face steps up to the hill: a wall
export const TOE_REACH = 16;       // how far out a bank or a face is looked for at all
export const DECK_GAP_T = 3;       // a crest this far above the ground is a structure: no bank
export const EARTH_T: Rgb = [0.42, 0.34, 0.26];
...
export const CUT_CLEAR = 0.04;    // how far below the deck floor the cut plane sits
...
export const SEA_BED = 6;
/** Every built terrain tile overlapping a world rectangle, marked for rebuild. */
export const AREA_MIX = 0.5;               // how far the ramp is pulled, after the raster's own tint
...
export const mmKey = (x: number, z: number): string => `${Math.round(x * 1000)},${Math.round(z * 1000)}`;
...
export function mmIndex(row: ArrayLike<number>): Map<string, MmPt> {
  const m = new Map<string, MmPt>();
  for (let i = 0; i + 2 < row.length; i += 3) m.set(mmKey(row[i], row[i + 1]), [row[i], row[i + 1], row[i + 2]]);
  return m;
}
export function mmNear(idx: Map<string, MmPt>, x: number, z: number, tol = 1.5e-3): MmPt | undefined {
...
/** Is (x, z) on the tile's boundary — one of its four edges, within it? */
export function onTileEdge(t: HeightTile, x: number, z: number): boolean {
  const e = 1e-3;
...
export function splitPoly(poly: Poly, L: BreakLine, x0: number, x1: number, z0: number, z1: number, eps: number): Poly[] {
...
/** Does a segment touch an axis-aligned box? (Liang–Barsky.) */
export function segTouchesBox(L: BreakLine, x0: number, x1: number, z0: number, z1: number): boolean {
...
export function stripFloor(s: StripLike, x: number, z: number): { y: number; out: number } {
...
export function stripsNear(S: TerrainStore, x: number, z: number, R: number, into: Set<StripLike>): void {
...
export function roadFloorHard(S: TerrainStore, x: number, z: number, wash = S.cutWash): number | null {
  cutSet.clear();
 ...
 *  wedge meets the ground: 0 at the crest already, CUT_REACH_M for a wall,
 *  -1 where the road stands clear of the ground (a structure: no bank). */
export function toeOut(S: TerrainStore, cx: number, cz: number, ox: number, oz: number, y: number): number {
...
export function stripBreakLines(S: TerrainStore, s: StripLike): BreakLine[] {
  if (s.bl) return s.bl;
  const out: BreakLine[] = [];
  s.bl = out;
  s.reach = s.hw + 0.6;
...
/** The corridor profile at a point: the height the terrain takes there and
 *  what it is — 0 ground, 1 floor, 2 cut face, 3 fill bank. */
export function corridorH(S: TerrainStore, x: number, z: number, N: number, cands?: Iterable<StripLike>): { h: number; k: number } {
 ...
 *  coordinate, so the neighbouring cell — split by the same line, from its own
 *  pieces — arrives at the same point bit for bit. */
export function cellTable(pos: Float32Array, idx: Uint32Array | Uint16Array, SEG: number): CellTris {
...
export function refineTileGeometry(S: TerrainStore, t: HeightTile, SEG: number, corridor: boolean): RefinedMesh | null {
...
export function carveCorridors(S: TerrainStore, t: HeightTile, pos: Float32Array, ct: CellTris, SEG: number): void {
  const t0 = performance.now();
  const cell = t.w / SEG;
...
export function channelsNear(S: TerrainStore, x: number, z: number, into: Set<StripLike>): void {
...
export const chanSet = new Set<StripLike>();
/**
 ...
 * sit on and the bore to pass through.
 */
export function channelFloorAt(S: TerrainStore, x: number, z: number, ceiling: number): number | null {
  chanSet.clear();
...
export function carveChannels(S: TerrainStore, t: HeightTile, pos: Float32Array, SEG: number): void {
...
export function storeBorder(S: TerrainStore, t: HeightTile, pos: Float32Array): void {
  const cxm = t.xs + t.w / 2, czm = t.zs + t.h / 2;
  const b: number[] = [];
  for (let v = 0; v < (pos.length / 3); v++) {
...
export function refinedBorderPins(S: TerrainStore, t: HeightTile, pos: Float32Array): Array<[number, number]> {
...
/** Does the tile at `nk` already carry every point of `t`'s border along
 *  their shared edge, at the same heights? */
export function borderShared(S: TerrainStore, t: HeightTile, nk: string): boolean {
...
export function normalMapBytes(S: TerrainStore, t: { w: number; data: Float32Array; xs?: number; zs?: number; h?: number }): Uint8Array {
  const W = 256;
  const mpp = t.w / W;
...
export function plainLattice(t: HeightTile, SEG: number): { pos: Float32Array; uv: Float32Array; idx: Uint32Array; cellTris: CellTris } {
  const n = (SEG + 1) * (SEG + 1);
  const pos = new Float32Array(n * 3), uv = new Float32Array(n * 2);
  const cw = t.w / SEG, ch = t.h / SEG;
  for (let iz = 0; iz <= SEG; iz++) for (let ix = 0; ix <= SEG; ix++) {
...
export function vertexNormals(pos: Float32Array, idx: Uint32Array | Uint16Array): Float32Array {
...
export function buildTile(S: TerrainStore, t: HeightTile, SEG: number, corridor: boolean, refine: boolean): TileBuild {
