...
// this test on the version line alone — which says nothing about the campaign
// and trains you to edit the test. Built separately from the handler so the
...
check('the file declares a version', Number.isInteger(authored.v) && authored.v > 0, authored.v);

// ── the route ──
const res = await call(`/~/campaign/${authored.v}`);
...
const old = await call(`/~/campaign/${authored.v + 3}`);
...
check('…and the route serves that version and no other', served.v === authored.v, [served.v, authored.v]);
...
// THE COURSE, where a leg has one. Generated by line-legs.mjs, so the failure
// mode is not a typo — it is a route that ran somewhere else. Two tiles of the
// overview never filled and the router picked its way round the hole, which
...
for (const l of served.legs ?? []) {
  if (!l.route) continue;
  courses++;
  const { pts, km, chordKm, vias } = l.route;
  check(`${l.id}: the course is an ordered polyline`,
    Array.isArray(pts) && pts.length >= 10
    && pts.every((p) => Array.isArray(p) && p.length === 2
      && Math.abs(p[0]) <= 90 && Math.abs(p[1]) <= 180 && !(p[0] === 0 && p[1] === 0)), pts?.length);
  check(`${l.id}: it STARTS at the giver`, hav(pts[0][0], pts[0][1], l.giver.lat, l.giver.lon) < 600,
    Math.round(hav(pts[0][0], pts[0][1], l.giver.lat, l.giver.lon)));
  const end = pts[pts.length - 1];
  check(`${l.id}: …and ENDS at the destination`, hav(end[0], end[1], l.dest.lat, l.dest.lon) < 600,
    Math.round(hav(end[0], end[1], l.dest.lat, l.dest.lon)));
  // The declared length has to be the length of the polyline that is actually
  // there — simplification shortens it a little, so a few percent is honest
  // and twenty is a course that was regenerated without its number.
  let walked = 0;
  for (let i = 1; i < pts.length; i++) walked += hav(pts[i - 1][0], pts[i - 1][1], pts[i][0], pts[i][1]);
  check(`${l.id}: the declared km is the polyline's own`, Math.abs(walked / 1000 - km) / km < 0.05,
    { km, walked: +(walked / 1000).toFixed(1) });
  // …AND IT IS A ROUTE, NOT A DETOUR ROUND A HOLE. Same gate line-legs.mjs
  // applies before writing; repeated here because the campaign is edited by
  // hand as well as generated, and this is the last place to catch it.
  check(`${l.id}: it follows the road without wandering`, km / chordKm < 1.6, { km, chordKm });
  check(`${l.id}: it names the roads that carry it`,
    Array.isArray(vias) && vias.length > 0 && vias.every((v) => typeof v.name === 'string' && v.km > 0),
    vias?.length);
}
check('at least one leg is a COURSE and not just a destination', courses >= 1, courses);
