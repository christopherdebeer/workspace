...
// Roads stream on their own schedule and a spawn can sit in a field for a
// while, so this is asked more than once rather than declared flaky.
let onRoad = { ok: false };
for (let i = 0; i < 6 && !onRoad.ok; i++) {
  if (i) await d.page.waitForTimeout(10000);
  onRoad = await d.page.evaluate(() => window.__toroad(600));
}
...
// A trunk road running north from THE TRUCK to 30km out, with a deliberate
// kink so a solved plan cannot be mistaken for a straight-line fallback.
//
// FROM THE TRUCK, because that is what a real overview tile holds. The first
// version started the way out at the streaming budget and made no portals —
// correctly: the survey had petered out around 250m and the synthetic road
// began at 1387m, so there was a genuine kilometre of unknown ground between
// them and inventing a road across it is exactly what a router must not do.
// Real coarse tiles cover everything including the ground under the wheels;
// what decides where the coarse tier BEGINS is the ingest gate, not the data,
// and that gate is the thing under test.
...
check('…and the plan reaches past the survey',
  res.route.km * 1000 > res.route.graph.inner * 4,
  { km: res.route.km, inner: res.route.graph.inner });
...
check('…and the driving line stops at the handover',
  res.route.fineKm * 1000 < res.route.graph.inner * 3, res.route);
...
check('without the coarse tier the far goal falls short again',
  off.pts === 0 || off.km < res.route.km / 3, { off: off.km, on: res.route.km });
