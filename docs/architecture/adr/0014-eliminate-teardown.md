...
| 2 | lit/canvas **bespoke membership + ordering** loaders | 🟢 **client done** — `members` now surfaces each member's placing decoration `{seq, fold}` (the missing piece), and lit's **client `loadDoc`** routes through `workspace.members` — its `_doc/` scan + per-key fetch deleted. **SSR stays direct-DDB by design** (a deliberate sibling: the cell reads its owner partition directly, no mid-SSR API round-trip — like canvas's direct storage). canvas membership is its own surface, tracked separately. | lit doc render parity (SSR ⇄ client) |
