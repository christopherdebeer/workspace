...
**The substrate-level vocabulary** — `entity × renderer × decoration`, the doc/board/view-tile/frame modality table, the renderer ladder (built-in → renderer-facts `_renderers/<type>` → cell iframes), the per-surface decoration grammar (canvas `{x,y,w,h}` / doc `{seq, fold}` / frame `region`) — survives as substantive content of the Projection.present stage. ADR-0012 (present-stage) and ADR-0015 (frames) stand as documentation of the present stage. Frames are not a new noun; they are a region-modality preset of present. **A correction surfaced by verification (2026-07): `resolvePresent` is exported (`platform/runtime/present.ts:52-60`) but is DEAD as a server call site — zero non-test, non-reexport callers; it ships only as cell-SDK surface. It is NOT invoked at the gateway `$types` boundary; that boundary uses `buildTypeVocabulary` (`services/gateway/service.ts:490-509`), and the server per-fact affordance path is `affordancesForTypes` → `resolveType` (`services/workspace/shared.ts`). Clients hand-roll their own affordance composition (e.g. `cells/canvas/client/lib/network/storage.ts` references the "gateway-resolved Present facet" and re-derives the icon locally). The factoring is real as a *facet convention* but there is no single shared present *function*; `frame.ts` (`regionBBox`/`fitRegion`) is the cleanest direct evidence of region-modality at the present stage. See `platform-reference/coherence-audit.md` (present-affordance seam).**
...
**Evidence (verified).** `platform/runtime/present.ts:17-26` (Affordance shape: icon/label/render/handlers) · `platform/runtime/present.ts:52-60` (resolvePresent) · `services/gateway/service.ts:490-509` (`buildTypes`→`buildTypeVocabulary` — how the gateway actually resolves the `$types` present facet; NOT `resolvePresent`. NB: `service.ts:335-339` is the unrelated `CORE_FACT_VERBS` set, mis-cited in earlier drafts) · `cells/canvas/shared/frame.ts:72-104` (regionBBox + fitRegion + resolveCamera — region-modality at present) · `docs/architecture/adr/0012-present-stage.md` · `docs/architecture/adr/0015-frames.md`.
...
- **Surface factoring (item-surface) as standalone:** folded into **Projection pipeline** as the structure of its present-stage output — the present stage is real as a *facet convention* (`present.{icon,label,render}`), though `resolvePresent` is dead server-side and the live paths are `buildTypeVocabulary` (gateway `$types`) and `affordancesForTypes`→`resolveType` (per-fact), with client consumer convergence partial; `entity × renderer × decoration` is the shape of present's output; ADR-0015's claim "every surface = collection × render modality" is the claim that present parameterises over modality.
