...
| **Delivery reliability under reactions.** The `spawn_children` multi-write fix and the `models.decide` concurrent-vote-child finding are instances of at-least-once/ordering behavior under the depth cap. | being designed out | **ADR-0018** (stateless stepper) — the deterministic `auto` prefix advances in-process with **no per-hop fact-write**, removing the cascade surface where drops bite; reliability worry remains only at genuine (model-paced) yields. ADR-0011 open item stands for those. |
| **`models.decide` concurrency.** Concurrent vote-child decisions don't auto-complete (decide action + barrier are correct). | being designed out | **ADR-0018** — with cell reads (ADR-0017) the join barrier becomes deterministic (the cell reads the children and advances the parent itself), retiring the `models.decide` completion dependency. Synthesis *content* may still use a model. |
| **No loop/step/timeout guard.** A cyclic reactive machine can thrash; a stuck branch parks a run forever. | partial (the `dryRun` `cycle` warning; `step()` reports `kind:'cycle'`/`'blocked'` rather than spinning; the reactor's revision depth cap) | **ADR-0018** (`step` halts on a guardless auto-loop and on a deterministic stall). A per-run wall-clock/step-count timeout is still unbuilt. |
| **Cell-side substrate reads.** The machine cell was built write-only; the agentic join barrier exists only because it could not read its own children. | foundation set | **ADR-0017** (Cell substrate access) — one shared `read`/`query`/`emit`/`supersede` client; `@c15r/models` already reads this way. Unblocks the deterministic barrier and `step`. |
...
- **v5.2 (current)** — **stateless stepper core** (ADR-0018): pure `step(run, machine, now)` in `engine.ts` advances the deterministic `auto` prefix in-process (CEL-guarded, one returned write, yields at non-`auto` rails / terminal / stall / cycle) + 10 unit tests. Foundation for a deterministic join barrier set by **ADR-0017** (one shared cell substrate-access client; cel-js now declared in the cell's `imports.json`). Shell wiring (the `step` tool, projection rework, in-process barrier) is the next increment — see §13.
