...
| **Shared client is URL-vendored, not a published package.** Consumers snapshot `/@c15r/kernel/substrate.js` at build time, so updating it means redeploy-kernel-then-rebuild-consumers (and a deploy-order edge). | bridge | **ADR-0017** packaging — publish `@c15r/substrate` to npm and import by bare specifier (esm.sh, version-pinned) to remove the snapshot/order coupling. |
...
- **v5.3 (current)** — **stepper wired + shared substrate client** (ADR-0017/0018). The kernel cell now serves a server-side substrate client at `/@c15r/kernel/substrate.js` (`read`/`query`/`emit`/`supersede`, organ-attested writes with a `FailedEntryCount` check); the machine cell imports it by URL and gains a `step` tool: read → pure `step` → one idempotent advance, with section/vote **spawn** and a **deterministic** in-process join barrier (`barrierAdvance` — reads siblings, advances the parent once all are done, no model). New reactive mode `reactive:"step"` projects ONE step subscription (+ the agent/work model deliveries), retiring the per-auto-rail cascade and the agentic join; `reactive:true` stays as the legacy path. 5 tools now. Deploy order: redeploy `@c15r/kernel` (serves `/substrate.js`) before rebuilding `@c15r/machine`.
