...
- **`workspace.graph` is the known fragile seam.** The full derived-edge projection runs server-side with no cursor regardless of caller limit; the 2 GB / 120 s workspace bump (and 512 MB / 150 s gateway, which must outlast it) are hotfixes, not the fix.
