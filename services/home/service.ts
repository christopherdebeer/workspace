...
import { defineService, ServiceHttpResponse, getOptional } from '../../platform/runtime';
...
export const handler = defineService({
  ...
  http: [
    ...
    // Live platform catalog (cell manifests), injected at deploy time. The SPA
    // fetches this so the rendered service list reflects what's actually wired.
    {
      method: 'GET',
      path: '/_catalog',
      handler: (): ServiceHttpResponse => ({
        statusCode: 200,
        headers: { 'cache-control': 'no-cache' },
        body: { services: JSON.parse(getOptional('PLATFORM_CATALOG') ?? '[]') },
      }),
    },
  ],
});
